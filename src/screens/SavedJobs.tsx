...
export default function SavedJobs() {
  ...
  const handleApply = (job: Job) => {
    navigation.navigate('ApplyForm', { job, fromScreen: 'SavedJobs' });
  };
  ...
  return (
    <View className={`flex-1 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <View className="px-5 pt-6 pb-4 flex-row items-center justify-between">
        ...
        <View className={`p-3 rounded-full ${isDarkMode ? 'bg-gray-800' : 'bg-white shadow-sm border border-gray-100'}`}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center justify-center mt-20 px-8">
            ...
            <View className={`p-6 rounded-full mb-5 ${isDarkMode ? 'bg-gray-800' : 'bg-blue-50'}`}>
              <Ionicons name="bookmark-outline" size={56} color={isDarkMode ? '#4b5563' : '#3b82f6'} />
            </View>
            
            <Text className={`text-xl font-bold text-center ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
              No saved jobs yet
            </Text>
            
            <Text className={`text-base mt-2 text-center leading-6 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              Jobs you save will appear here. Find your dream job and bookmark it for later!
            </Text>
            ...
            >
              ...
              <Text className="text-white text-base font-bold ml-2">Browse Jobs</Text>
            </TouchableOpacity>
          </View>
        }
      />
