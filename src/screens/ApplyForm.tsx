...
export default function ApplyForm() {
  ...
  const validateForm = () => {
    ...
    if (!email.trim()) {
      ...
      newErrors.email =
        "Please enter a valid email format (e.g., user@mail.com)";
      valid = false;
    }
    ...
    if (!contact.trim()) {
      ...
      newErrors.contact =
        "Please enter a valid 10-11 digit numeric contact number";
      valid = false;
    }
  ...
  return (
    ...
    >
      ...
      >
        {/* Header Summary Card */}
        <View
          className={`mb-8 p-6 rounded-3xl border ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-blue-50 border-blue-100"}`}
        >
          <View className="flex-row items-center mb-2">
            ...
            />
            <Text
              className={`ml-2 text-lg font-bold ${isDarkMode ? "text-blue-400" : "text-blue-700"}`}
            >
              Applying For
            </Text>
          </View>
          <Text
            className={`text-2xl font-extrabold mt-1 ${isDarkMode ? "text-white" : "text-gray-900"}`}
          >
            {job.title}
          </Text>
          <Text
            className={`text-base font-medium mt-1 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
          >
            at {job.company}
          </Text>
        </View>
