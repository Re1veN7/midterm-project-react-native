...
export default function JobCard({
  ...
  const { savedJobs, saveJob, removeSavedJob, appliedJobs, isDarkMode } =
    useJobStore();
  ...
  const handleSave = () => {
    Alert.alert("Save Job", "Do you want to save this job?", [
  ...
  const handleRemove = () => {
    Alert.alert("Remove Job", "Do you want to remove this job?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Yes",
        style: "destructive",
  ...
  return (
    <View
      className={`p-5 m-2 mx-4 rounded-2xl shadow-sm border ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}
    >
      {/* Top Row: Job Title */}
      ...
      <View className="mb-5">
        <View className="flex-row items-center mb-2">
          ...
          />
          <Text
            className={`ml-2 text-base ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}
          >
        ...
        <View className="flex-row items-center mt-1">
          ...
          {job.salary ? (
            <View className="flex-row items-center">
              ...
              />
              <Text
                className={`ml-1 font-semibold ${isDarkMode ? "text-green-400" : "text-green-600"}`}
              >
                {job.salary}
              </Text>
            </View>
      ...
      >
        ...
        {isSavedScreen ? (
          ...
          >
            ...
            />
            <Text
              className={`ml-2 font-bold ${isDarkMode ? "text-red-400" : "text-red-600"}`}
            >
              Remove
            </Text>
          </TouchableOpacity>
          ...
          >
            ...
            />
            <Text
              className={`ml-2 font-bold ${isSaved ? (isDarkMode ? "text-gray-400" : "text-gray-500") : "text-blue-500"}`}
            >
              {isSaved ? "Saved" : "Save"}
            </Text>
          </TouchableOpacity>
